  ...
  <q-layout view="hHh LpR fFf">
    <q-header elevated class="bg-bl-1 text-white">
      <div class="row wrap justify-between items-center content-center" style="height: 50px">
        ...
        <div class="col-7">
          <q-tabs shrink>
            <q-route-tab @click="tLPanel = true, tRPanel = true, bLPanel = true, bRPanel = true" class="top-tab-bg"
              name="tabSelfNav" label="自主导航" :to="{ path: '/SelfNavigation' }" exact replace />
            <q-route-tab @click="tLPanel = true, tRPanel = true, bLPanel = true, bRPanel = true" class="top-tab-bg"
              name="tabScenePer" label="场景感知" :to="{ path: '/ScenePerception' }" exact replace />
            <q-route-tab @click="tLPanel = true, tRPanel = true, bLPanel = true, bRPanel = true" class="top-tab-bg"
              name="tabMonitor" label="视频监控" :to="{ path: '/Monitor' }" exact replace />
            <q-route-tab @click="tLPanel = true, tRPanel = true, bLPanel = true, bRPanel = true" class="top-tab-bg"
              name="tabWaypointPlan" label="百度地图" :to="{ path: '/WaypointPlan' }" exact replace />
            <q-route-tab @click="tLPanel = false, tRPanel = false, bLPanel = false, bRPanel = false" class="top-tab-bg"
              name="tabOlTest" label="航道地图" :to="{ path: '/OlTest' }" exact replace />
          </q-tabs>
        </div>
        <!-- home tab -->
...
export default defineComponent({
  ...
  methods: {
    goHome() {
      this.tLPanel = true; this.tRPanel = true; this.bLPanel = true; this.bRPanel = true;
      this.$router.replace({
        path: "/HomePage",
      });
    },
